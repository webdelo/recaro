...
(function($, undefined) {
	window.touchSlider = function(options) {
		...
		crossLeft(slides.eq(0).addClass('current'), 0);
			...
			return {
				...
				to: function(toIndex, opt) {
					...
					var duration = options.duration,
						node = slides.removeClass('current').eq(toIndex).addClass('current'),
		...
		function changedView(index) {
			var elem$ = pagination.removeClass(options.currentClass)
				.eq(index).addClass(options.currentClass);
			if( elem$.parents('li').length > 0 )
				elem$.parents('li').click();
		}
